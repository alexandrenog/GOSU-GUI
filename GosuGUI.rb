...
class GosuLabel < GosuComponent
	...
	def initialize(window,position,dimension,text="",hasbackground=false,limit=1<<8,background=0xff555555,fontcolor=0xffffffff)
		...
		@limit=limit
		@text=text
		@textImg=Gosu::Image.from_text(@text.split("").first(@limit).join, @height,{font: Gosu.default_font_name})
	end
	def hasBackGround
...
class GosuButton < GosuComponent
	...
	def initialize(window,position,dimension,text,procedure,image=nil,background=0xff555555,fontcolor=0xffffffff)
		...
		@text=text
		@textImg=Gosu::Image.from_text((text)?(text):(""), @height, {font: Gosu.default_font_name}) # fontsize => height
	end
	def executeClick
...
class GosuTextField < GosuLabel
	...
	def update
		@text=@textinput.text
		@textImg=Gosu::Image.from_text(@text, @height, {font: Gosu.default_font_name})
	end
	def text=(v)
		@textinput.text=v
		update
